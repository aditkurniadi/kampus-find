    ...
    <nav aria-label="Global" class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
        <div class="flex lg:flex-1">
            <a href="/" class="-m-1.5 p-1.5">
                ...
                <span class="sr-only">Kampus Find</span>
            </a>
        </div>
        ...
        {{-- BAGIAN DESKTOP: LOGIKA LOGIN/LOGOUT --}}
        <div class="hidden lg:flex lg:flex-1 lg:justify-end lg:gap-x-6">
            @auth
                {{-- Jika User Login: Tampilkan Nama & Tombol Logout --}}
                <div class="flex items-center gap-4">
                    <a href="{{ route('dashboard') }}"
                        class="text-sm/6 font-semibold text-indigo-600 dark:text-indigo-400 hover:underline">
                        Hi, {{ Auth::user()->name }}
                    </a>
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="text-sm/6 font-semibold text-gray-900 dark:text-white hover:text-red-500 transition">
                            Log out
                        </button>
                    </form>
                </div>
            @else
                ...
                <a href="{{ route('login') }}" class="text-sm/6 font-semibold text-gray-900 dark:text-white">
                ...
                <el-dialog-panel
                    ...
                    <div class="mt-6 flow-root">
                        <div class="-my-6 divide-y divide-gray-500/10 dark:divide-white/10">
                            ...
                            {{-- BAGIAN MOBILE: LOGIKA LOGIN/LOGOUT --}}
                            <div class="py-6">
                                @auth
                                    {{-- Jika Login --}}
                                    <div class="mb-4 px-3 text-sm font-semibold text-indigo-500">
                                        Akun: {{ Auth::user()->name }}
                                    </div>
                                    ...
                                    <form method="POST" action="{{ route('logout') }}">
                                        @csrf
                                        <button type="submit"
                                            class="-mx-3 block w-full text-left rounded-lg px-3 py-2.5 text-base/7 font-semibold text-red-600 hover:bg-gray-50 dark:hover:bg-white/5">
                                            Log out
                                        </button>
                                    </form>
                                @else
